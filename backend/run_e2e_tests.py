...
def run_real_chat_flow_tests():
    ...
    result = subprocess.run([
        sys.executable, "-m", "pytest", 
        "tests/e2e/test_real_chat_flow.py", 
        "-v", "--tb=short"
    ])
...
def main():
    ...
    parser.add_argument("--with-server", action="store_true", 
                       help="Run tests with a real server")
    parser.add_argument("--mock-ai", action="store_true", 
                       help="Use mocked AI responses")
    parser.add_argument("--coverage", action="store_true",
                       help="Run with coverage report")
    parser.add_argument("--all", action="store_true",
                       help="Run all tests including server tests")
    parser.add_argument("--auth-only", action="store_true",
                       help="Run only authentication tests")
    parser.add_argument("--threads-only", action="store_true",
                       help="Run only threads tests")
    parser.add_argument("--signup-only", action="store_true",
                       help="Run only signup tests")
    parser.add_argument("--login-only", action="store_true",
                       help="Run only login tests")
    parser.add_argument("--logout-only", action="store_true",
                       help="Run only logout tests")
    parser.add_argument("--access-token-only", action="store_true",
                       help="Run only access token tests")
    parser.add_argument("--chat-flow-only", action="store_true",
                       help="Run only real chat flow tests")
