...
class ThreadRepository:
    ...
    async def create_thread(self, db: AsyncSession, params: CreateThreadParams) -> DBThread:
        ...
        thread = DBThread(
            ...
            **params.model_dump(exclude={"created_at", "updated_at", "resumed_at"}, exclude_none=True, exclude_unset=True)
        )
        db.add(thread)
        await db.flush()
        return thread
    ...
    async def get_user_threads(self, db: AsyncSession, params: GetDBUserThreadsParams) -> list[DBThread]:
    ...
    async def count_user_threads(self, db: AsyncSession, user_id: str) -> int:
        ...
        result = await db.execute(
            ...
            .where(DBThread.user_id == user_id)
        )
        return result.scalar_one()
    
    
    async def create_threads(self, db: AsyncSession, params: list[CreateThreadParams]) -> list[DBThread]:
        ...
        db_threads = [DBThread( 
            ...
            **thread.model_dump(exclude={"created_at", "updated_at", "resumed_at"}, exclude_none=True, exclude_unset=True)
