...
class Recipe(BaseModel):
    ...
    @staticmethod
    def from_db_recipe(recipe: DBRecipe) -> "Recipe":
        return Recipe(
            name=recipe.name,
            description=recipe.description,
            ingredients=[RecipeIngredient(**ing) for ing in recipe.ingredients] if recipe.ingredients else None,
            instructions=[RecipeInstruction(**inst) for inst in recipe.instructions] if recipe.instructions else None,
            categories=[RecipeCategory(**cat) for cat in recipe.categories] if recipe.categories else None,
...
class UserRecipe(Recipe):
    ...
    @staticmethod
    def from_db_recipe(recipe: DBRecipe) -> "UserRecipe":
        return UserRecipe(
            id=recipe.id,
            ...
            make_ahead_tips=recipe.make_ahead_tips,
            coordination_timeline=recipe.coordination_timeline,
            created_at=to_utc_isostring(recipe.created_at),
            updated_at=to_utc_isostring(recipe.updated_at)
        )
    ...
    def get_recipe(self) -> Recipe:
        return Recipe(
            ...
            ingredients=[RecipeIngredient(**ing) for ing in self.ingredients] if self.ingredients else None,
            instructions=[RecipeInstruction(**inst) for inst in self.instructions] if self.instructions else None,
            categories=[RecipeCategory(**cat) for cat in self.categories] if self.categories else None,
...
SimpleRecipeField = RecipeName | RecipeDescription | RecipePrepTime | RecipeCookTime | RecipeServings | RecipeChefNotes | RecipeSubstitutions | RecipeEquipmentAlternatives | RecipeScalingGuidance | RecipeStorageNotes | RecipeServingSuggestions | RecipeMakeAheadTips | RecipeCoordinationTimeline
NestedRecipeField = RecipeIngredient | RecipeInstruction | RecipeCategory

class RecipeField(BaseModel):
    name: Literal["name", "description", "servings", "chef_notes", "substitutions", "equipment_alternatives", "scaling_guidance", "storage_notes", "serving_suggestions", "make_ahead_tips", "coordination_timeline", "prep_time_minutes", "cook_time_minutes", "ingredient", "instruction", "category", "ingredients", "instructions", "categories"]
    value: str | int | RecipeIngredient | RecipeInstruction | RecipeCategory | list[RecipeIngredient] | list[RecipeInstruction] | list[RecipeCategory]
    
    @field_validator('name')
    @classmethod
    def validate_name(cls, v):
        if v not in ["name", "description", "servings", "chef_notes", "substitutions", "equipment_alternatives", "scaling_guidance", "storage_notes", "serving_suggestions", "make_ahead_tips", "coordination_timeline", "prep_time_minutes", "cook_time_minutes", "ingredient", "instruction", "category", "ingredients", "instructions", "categories"]:
    ...
    def validate_value(cls, v, info):
        ...
        if field_name in ["prep_time_minutes", "cook_time_minutes"]:
            if v is not None and not isinstance(v, int):
                raise ValueError(f"Field '{field_name}' expects an integer value, got {type(v).__name__}")
        
        # String fields
        elif field_name in ["name", "description", "servings", "chef_notes", "substitutions", 
                           "equipment_alternatives", "scaling_guidance", "storage_notes", 
                           "serving_suggestions", "make_ahead_tips", "coordination_timeline"]:
            if v is not None and not isinstance(v, str):
                raise ValueError(f"Field '{field_name}' expects a string value, got {type(v).__name__}")
        
        # Single object fields
        elif field_name == "ingredient" and v is not None and not isinstance(v, RecipeIngredient):
            raise ValueError(f"Field '{field_name}' expects a RecipeIngredient, got {type(v).__name__}")
        elif field_name == "instruction" and v is not None and not isinstance(v, RecipeInstruction):
            raise ValueError(f"Field '{field_name}' expects a RecipeInstruction, got {type(v).__name__}")
        elif field_name == "category" and v is not None and not isinstance(v, RecipeCategory):
            raise ValueError(f"Field '{field_name}' expects a RecipeCategory, got {type(v).__name__}")
        
        # List fields
        elif field_name == "ingredients" and v is not None:
            if not isinstance(v, list):
                raise ValueError(f"Field '{field_name}' expects a list of RecipeIngredient, got {type(v).__name__}")
            for item in v:
                if not isinstance(item, RecipeIngredient):
                    raise ValueError(f"Field '{field_name}' expects list items to be RecipeIngredient, got {type(item).__name__}")
        elif field_name == "instructions" and v is not None:
            if not isinstance(v, list):
                raise ValueError(f"Field '{field_name}' expects a list of RecipeInstruction, got {type(v).__name__}")
            for item in v:
                if not isinstance(item, RecipeInstruction):
                    raise ValueError(f"Field '{field_name}' expects list items to be RecipeInstruction, got {type(item).__name__}")
        elif field_name == "categories" and v is not None:
            if not isinstance(v, list):
                raise ValueError(f"Field '{field_name}' expects a list of RecipeCategory, got {type(v).__name__}")
            for item in v:
                if not isinstance(item, RecipeCategory):
                    raise ValueError(f"Field '{field_name}' expects list items to be RecipeCategory, got {type(item).__name__}")
...
class BaseRecipeParams(BaseModel):
    ...
    ingredients: list[RecipeIngredient] | None = Field(default=None, description="List of ingredients with quantities")
    instructions: list[RecipeInstruction] | None = Field(default=None, description="Step-by-step cooking instructions")
    categories: list[RecipeCategory] | None = Field(default=None, description="Recipe tags/categories")
    ...
    equipment_alternatives: str | None = Field(default=None, description="Alternative cooking tools")
    scaling_guidance: str | None = Field(default=None, description="Instructions for adjusting quantities")
    storage_notes: str | None = Field(default=None, description="Leftover storage guidelines")
    serving_suggestions: str | None = Field(default=None, description="Accompaniments and presentation ideas")
    make_ahead_tips: str | None = Field(default=None, description="Advance preparation advice")
    coordination_timeline: str | None = Field(default=None, description="Multi-component coordination schedule")
