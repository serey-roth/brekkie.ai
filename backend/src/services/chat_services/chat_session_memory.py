...
class ChatSessionMemoryManager:
    ...
    async def get_user_profile(self, user_id: str, thread_id: str):
        user_profile_results = await self.store.asearch(("brekkie", user_id, "profile"), query="the user profile", limit=1)
    ...
    def update_user_profile_delayed(self, user_id: str, thread_id: str, user_message: str, delay: int = 10):
