...
class ChatSessionStore:
    ...
    async def update_thread(
        self, db: AsyncSession, params: UpdateThreadParams
    ) -> Thread:
        return await self.thread_service.update_thread(db, params)

    async def resume_thread(
        self, db: AsyncSession, params: ResumeThreadParams
    ) -> Thread | None:
        try:
    ...
    async def get_user_recipes(self, db: AsyncSession, user_access: UserAccess) -> list[UserRecipe]:
        return await self.recipe_service.get_user_recipes(db, user_access.user_id)
