...
class MessageCacheService:
    ...
    async def get_message(self, user_id: str, thread_id: str, message_id: str) -> Message | None:
        return await self.message_cache.get_json(self._get_message_key(user_id, thread_id, message_id), Message)
    ...
    async def get_messages(self, user_id: str, thread_id: str) -> list[Message]:
        return await self.message_cache.get_all_json_by_pattern(pattern=self._get_all_messages_key(user_id, thread_id), model=Message)
    
    
    async def get_messages_by_user_id(self, user_id: str) -> list[Message]:
        return await self.message_cache.get_all_json_by_pattern(pattern=self._get_all_user_messages_key(user_id), model=Message)
    

    async def get_messages_by_id(self, user_id: str, thread_id: str, message_ids: list[str]) -> list[Message]:
    ...
    async def create_user_message(self, user_id: str, params: CreateUserMessageParams, ttl: int | None = None) -> Message:
        message = await self.create_message(user_id, params, ttl)
        return message
    
    
    async def create_assistant_text_message(self, user_id: str, params: CreateAssistantTextMessageParams, ttl: int | None = None) -> Message:
        message = await self.create_message(user_id, params, ttl)
        return message
    
    
    async def create_assistant_recipe_message(self, user_id: str, params: CreateAssistantRecipeMessageParams, ttl: int | None = None) -> Message:
        message = await self.create_message(user_id, params, ttl)
        return message
    
    
    async def create_assistant_tool_message(self, user_id: str, params: CreateAssistantToolMessageParams, ttl: int | None = None) -> Message:
        message = await self.create_message(user_id, params, ttl)
        return message
    
    
    async def update_message(self, user_id: str, thread_id: str, params: UpdateMessageParams) -> Message:
        ...
        for key, value in params.model_dump(exclude={"id"}, exclude_none=True, exclude_unset=True).items():
            if key == "updated_at":
                new_message.updated_at = to_utc_isostring(value)
            else:
                setattr(new_message, key, value)
                
        previous_ttl = await self.message_cache.get_ttl(self._get_message_key(user_id, thread_id, params.id))
        if previous_ttl < 0:
    ...
    async def _sort_messages(self, messages: list[Message], sort_by: str, sort_order: str) -> list[Message]:
    ...
    async def _filter_messages(self, messages: list[Message], from_timestamp: datetime | None, sort_by: str, sort_order: str) -> list[Message]:
        copied_messages = [m.model_copy(deep=True) for m in messages]
        
        def filter_fn(m: Message) -> bool:
            if sort_by == "created_at" and sort_order == "asc":
                return datetime.fromisoformat(m.created_at).replace(tzinfo=timezone.utc) > from_timestamp
            elif sort_by == "created_at" and sort_order == "desc":
                return datetime.fromisoformat(m.created_at).replace(tzinfo=timezone.utc) < from_timestamp
            elif sort_by == "updated_at" and sort_order == "asc":
                return datetime.fromisoformat(m.updated_at).replace(tzinfo=timezone.utc) > from_timestamp
    ...
    async def get_paginated_messages(self, params: GetMessagesParams) -> PaginatedMessages:
        ...
        if params.from_timestamp:
            copied_messages = await self._filter_messages(copied_messages, params.from_timestamp, params.sort_by, params.sort_order)
