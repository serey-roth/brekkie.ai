...
class MessageService:
    ...
    def _to_message_dto(self, message: DBMessage) -> Message:
        ...
        return Message(
            ...
            tool_input=cast(dict[str, Any], message.tool_input)
            if message.tool_input is not None
            else None,
            tool_output=cast(dict[str, Any], message.tool_output)
    ...
    async def get_messages_by_ids(self, db: AsyncSession, message_ids: list[str]) -> list[Message]:
        logger.debug(f"Getting messages by ids {message_ids}")
        db_messages = await self.repository.get_messages_by_ids(db, message_ids)
        return [self._to_message_dto(message) for message in db_messages]
    ...
    async def get_paginated_messages(
        ...
        message_responses = [self._to_message_dto(message) for message in messages_to_return]
        return PaginatedMessages(
            messages=message_responses,
            total_count=await self.repository.count_messages(
                db, CountMessagesParams(thread_id=params.thread_id)
            ),
            has_more=has_more,
    ...
    async def count_messages(self, db: AsyncSession, params: CountMessagesParams) -> int:
        logger.debug(f"Counting messages for thread {params.thread_id}")
        count = await self.repository.count_messages(db, params)
