...
class RecipeCacheService:
    ...
    async def get_recipe(self, user_id: str, thread_id: str, recipe_id: str) -> UserRecipe | None:
        return await self.recipe_cache.get_json(self._get_recipe_key(user_id, thread_id, recipe_id), UserRecipe)
    ...
    async def get_recipes(self, user_id: str, thread_id: str) -> list[UserRecipe]:
        return await self.recipe_cache.get_all_json_by_pattern(pattern=self._get_all_recipes_key(user_id, thread_id), model=UserRecipe)


    async def get_recipes_by_user_id(self, user_id: str) -> list[UserRecipe]:
        return await self.recipe_cache.get_all_json_by_pattern(pattern=self._get_all_user_recipes_key(user_id), model=UserRecipe)
    

    async def get_recipes_by_ids(self, user_id: str, thread_id: str, recipe_ids: list[str]) -> list[UserRecipe]:
    ...
    async def update_recipe_field(self, user_id: str, thread_id: str, params: UpdateRecipeFieldParams) -> UserRecipe:
        ...
        if field_name in ["ingredient", "instruction", "category"]:
            if field_name == "ingredient": 
                new_recipe.ingredients = [field_value] if new_recipe.ingredients is None else new_recipe.ingredients + [field_value]
            elif field_name == "instruction":
                new_recipe.instructions = [field_value] if new_recipe.instructions is None else new_recipe.instructions + [field_value]
            elif field_name == "category":
                new_recipe.categories = [field_value] if new_recipe.categories is None else new_recipe.categories + [field_value]
        
        elif field_name in ["ingredients", "instructions", "categories"]:
            if field_name == "ingredients":
                new_recipe.ingredients = field_value
            elif field_name == "instructions":
                new_recipe.instructions = field_value
            elif field_name == "categories":
                new_recipe.categories = field_value
        ...
        previous_ttl = await self.recipe_cache.get_ttl(self._get_recipe_key(user_id, thread_id, params.id))
        if previous_ttl < 0:
    ...
    def _ensure_models(self, data: Optional[list[Any]], model_cls: Type[BaseModel]) -> list[BaseModel]:
        if not data:
            return []
        return [
    ...
    async def update_recipe(self, user_id: str, thread_id: str, params: UpdateRecipeParams) -> UserRecipe:
        recipe = await self.get_recipe(user_id, thread_id, params.id)
        if recipe is None:
        ...
        if params.ingredients is not None:
            new_recipe.ingredients = self._ensure_models(params.ingredients, RecipeIngredient)
        if params.instructions is not None:
            new_recipe.instructions = self._ensure_models(params.instructions, RecipeInstruction)
        if params.categories is not None:
            new_recipe.categories = self._ensure_models(params.categories, RecipeCategory)
            
        for key, value in params.model_dump(exclude={"id", "updated_at", "ingredients", "instructions", "categories"}, exclude_unset=True, exclude_none=True).items():
