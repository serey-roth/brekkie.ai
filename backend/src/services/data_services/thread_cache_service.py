...
class ThreadCacheService:
    ...
    async def set_thread(self, thread: Thread, ttl: int | None = None, keep_ttl: bool = False) -> None:
        ...
        await self.thread_cache.set_json(self._get_thread_key(thread.user_id, thread.id), thread.model_dump(mode="json"), ttl=set_ttl, keep_ttl=keep_ttl)
    
    
    async def get_threads(self, user_id: str) -> list[Thread]:
        return await self.thread_cache.get_all_json_by_pattern(pattern=self._get_all_threads_key(user_id), model=Thread)
    ...
    def _filter_threads(self, threads: list[Thread], from_timestamp: datetime | None, sort_by: str, sort_order: str) -> list[Thread]:
        copied_threads = [t.model_copy(deep=True) for t in threads]
        
        def filter_fn(t: Thread) -> bool:
            if sort_by == "created_at" and sort_order == "asc":
                return datetime.fromisoformat(t.created_at).replace(tzinfo=timezone.utc) > from_timestamp
            elif sort_by == "created_at" and sort_order == "desc":
                return datetime.fromisoformat(t.created_at).replace(tzinfo=timezone.utc) < from_timestamp
            elif sort_by == "updated_at" and sort_order == "asc":
                return datetime.fromisoformat(t.updated_at).replace(tzinfo=timezone.utc) > from_timestamp
    ...
    async def get_paginated_threads(self, params: GetUserThreadsParams) -> PaginatedThreads:
        ...
        if params.from_timestamp:
            copied_threads = self._filter_threads(copied_threads, params.from_timestamp, params.sort_by, params.sort_order)
