...
class RedisCache(Generic[T]):
    ...
    async def set(
        self, key: str, value: str, ttl: int | None = None, keep_ttl: bool = False
    ) -> None:
    ...
    async def set_json(
        self, key: str, value: T, ttl: int | None = None, keep_ttl: bool = False
    ) -> None:
        if ttl is not None and keep_ttl:
            raise ValueError("ttl and keep_ttl cannot be used together")
        ...
        if keep_ttl:
            ...
            )  # redis will use the ttl from the previous value
        elif ttl is not None:
            await self.redis.set(
                key, json_value, ex=ttl
            )  # redis will reset ttl when value is updated
        else:
    ...
    async def get_json(self, key: str, model: Type[T]) -> T | None:
        ...
        if raw:
            ...
            return (
                model(**parsed) if callable(model) else parsed
            )  # model can be dict or pydantic class
        return None
