...
class StreamingRecipeFieldParser:
    ...
    def tag_handlers(self) -> dict[str, Callable[[str, str], RecipeField | None]]:
        ...
        return {
            ...
            ## Child fields
            "ing_name": lambda tag, text: self._handle_child_field(parent_tag="ingredient", child_tag=tag, text=text),
            "ing_quantity": lambda tag, text: self._handle_child_field(parent_tag="ingredient", child_tag=tag, text=text),
            "ing_unit": lambda tag, text: self._handle_child_field(parent_tag="ingredient", child_tag=tag, text=text),
            "ins_title": lambda tag, text: self._handle_child_field(parent_tag="instruction", child_tag=tag, text=text),
            "ins_description": lambda tag, text: self._handle_child_field(parent_tag="instruction", child_tag=tag, text=text),
            "cat_name": lambda tag, text: self._handle_child_field(parent_tag="category", child_tag=tag, text=text),
    ...
    def _safe_parse(self, tag: str, xml_fragment: str) -> RecipeField | None:
        ...
        try:
            ...
            for _, elem in parser.read_events():
                ...
                if elem_tag != tag or elem_tag not in self.known_tags:
                    continue
    ...
    def _handle_child_field(self, parent_tag: Literal["ingredient", "instruction", "category"], child_tag: str, text: str) -> None:
        """
    ...
    def _handle_nested_field(self, name: Literal["ingredient", "instruction", "category"]) -> RecipeField | None:
        """
        ...
        match name:
            case "ingredient":
                ...
                self.recipe.ingredients.append(RecipeIngredient(
            ...
            case "instruction":
                ...
                self.recipe.instructions.append(RecipeInstruction(
            ...
            case "category":
                ...
                self.recipe.categories.append(RecipeCategory(
    ...
    def _handle_container_field(self, name: Literal["ingredients", "instructions", "categories"]) -> RecipeField | None:
        """
