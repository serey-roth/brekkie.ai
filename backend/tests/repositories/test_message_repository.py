...
class TestCreateMessage:
    async def test_create_message(self, async_session: AsyncSession, message_repository: MessageRepository):
        """Test creating a basic message with required fields only."""
...
class TestGetMessage:
    ...
    async def create_message_in_db(self, async_session: AsyncSession, message_repository: MessageRepository, sample_message: dict):
        params = CreateMessageParams(**sample_message)
        await message_repository.create_message(async_session, params)
        await async_session.commit()
        
        
    async def test_get_message(self, async_session: AsyncSession, message_repository: MessageRepository, create_message_in_db, message_id: str, sample_message: dict):
    ...
    def sample_messages(self, user_id: str, thread_id: str, recipe_id: str):
        ...
        return [
            ...
            },
            {
                ...
                "tool_input": {"ingredients": ["chicken breast", "broccoli", "carrots", "onion"], "cooking_time": "30 minutes", "difficulty": "easy"},
...
class TestUpdateMessage:
    ...
    async def test_update_existing_message(self, async_session: AsyncSession, message_repository: MessageRepository, sample_message: dict, create_message_in_db):
    ...
    async def test_update_message_with_none_values(self, async_session: AsyncSession, message_repository: MessageRepository, sample_message: dict, create_message_in_db):
        """Test updating a message with None values preserves original values."""
        params = UpdateMessageParams(
            id=sample_message["id"],
            updated_at=datetime.now(timezone.utc) + timedelta(seconds=50),
            text_content=None,
...
class TestCountThreadMessages:
    ...
    @pytest.fixture(scope="function")
    def sample_messages(self, user_id: str, thread_id: str):
...
class TestCreateMessages:
    @pytest_asyncio.fixture(scope="function")
    async def create_messages_in_db(self, async_session: AsyncSession, message_repository: MessageRepository, sample_messages: list[dict]):
