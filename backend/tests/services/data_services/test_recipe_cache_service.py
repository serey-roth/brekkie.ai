...
def sample_recipe(user_id: str, thread_id: str, recipe_id: str) -> UserRecipe:
    return UserRecipe(
        ...
        ingredients=[RecipeIngredient(name="ingredient_name", quantity="1", unit="unit")],
        instructions=[
            RecipeInstruction(title="instruction_title", description="instruction_description")
        ],
        categories=[RecipeCategory(name="category_name")],
...
class TestGetAndSetRecipe:
    def test_get_recipe_key(self, recipe_cache_service: RecipeCacheService) -> None:
    ...
    def test_get_all_recipes_key(self, recipe_cache_service: RecipeCacheService) -> None:
    ...
    def test_get_all_user_recipes_key(self, recipe_cache_service: RecipeCacheService) -> None:
        key = recipe_cache_service._get_all_user_recipes_key("user_id")
        assert key == "brekkie:chat_session:user_id:threads:*:recipes:*"

    @pytest.mark.asyncio
    async def test_set_and_get_recipe(
        self,
    ...
    @pytest.mark.asyncio
    async def test_set_and_get_recipe_with_ttl(
        self,
        recipe_cache_service: RecipeCacheService,
        sample_recipe: UserRecipe,
        user_id: str,
    ...
    @pytest.mark.asyncio
    async def test_create_recipe_with_no_recipe_fields(
        self,
        recipe_cache_service: RecipeCacheService,
        sample_recipe: UserRecipe,
        user_id: str,
    ...
    @pytest.mark.asyncio
    async def test_create_recipe_with_recipe_fields(
        self,
        recipe_cache_service: RecipeCacheService,
        sample_recipe: UserRecipe,
        user_id: str,
    ...
    @pytest.mark.asyncio
    async def test_create_recipe_with_ttl(
        self,
        recipe_cache_service: RecipeCacheService,
        sample_recipe: UserRecipe,
        user_id: str,
    ...
    @pytest.mark.asyncio
    async def test_get_recipes_by_user_id(
        self, recipe_cache_service: RecipeCacheService, sample_recipe: UserRecipe, user_id: str
    ) -> None:
        first_recipe = sample_recipe.model_copy(
            update={"id": "first_recipe_id", "thread_id": "first_thread_id"}
        )
        second_recipe = sample_recipe.model_copy(
            update={"id": "second_recipe_id", "thread_id": "second_thread_id"}
        )
        third_recipe = sample_recipe.model_copy(
            update={"id": "third_recipe_id", "thread_id": "third_thread_id"}
        )
        ...
        recipes = await recipe_cache_service.get_user_recipes(user_id)
        assert_deep_equal(recipes, [first_recipe, second_recipe, third_recipe])


class TestUpdateRecipeField:
    @pytest.mark.asyncio
    async def test_update_recipe_simple_field(
        self,
        recipe_cache_service: RecipeCacheService,
        sample_recipe: UserRecipe,
        user_id: str,
        ...
        assert_deep_equal(
            ...
            sample_recipe.model_copy(
                update={"name": "new_name", "updated_at": to_utc_isostring(updated_at)}
            ),
        )

    @pytest.mark.asyncio
    async def test_add_first_ingredient(
        self,
        recipe_cache_service: RecipeCacheService,
        sample_recipe: UserRecipe,
        user_id: str,
        ...
        await recipe_cache_service.set_recipe(
            UserRecipe(
                id=recipe_id,
                user_id=user_id,
                thread_id=thread_id,
        ...
        first_ingredient = RecipeIngredient(name="new_ingredient_name", quantity="1", unit="unit")
    ...
    @pytest.mark.asyncio
    async def test_add_second_ingredient(
        self,
        recipe_cache_service: RecipeCacheService,
        sample_recipe: UserRecipe,
        user_id: str,
        ...
        await recipe_cache_service.set_recipe(
            UserRecipe(
                ...
                updated_at=to_utc_isostring(datetime.now(timezone.utc)),
                ingredients=[
                    RecipeIngredient(name="first_ingredient_name", quantity="1", unit="unit")
                ],
            )
        )

        second_ingredient = RecipeIngredient(
            name="second_ingredient_name", quantity="2", unit="unit"
        )
    ...
    async def test_add_instruction(
        ...
        first_instruction = RecipeInstruction(
            title="first_instruction_title", description="first_instruction_description"
        )
    ...
    async def test_add_category(
        ...
        first_category = RecipeCategory(name="first_category_name")
    ...
    @pytest.mark.asyncio
    async def test_update_non_existent_recipe(
        self, recipe_cache_service: RecipeCacheService, user_id: str, thread_id: str, recipe_id: str
    ) -> None:
        params = UpdateRecipeFieldParams(
            id=recipe_id,
            field=RecipeField(
    ...
    @pytest.mark.asyncio
    async def test_update_recipe_with_invalid_field_value(
        self,
        recipe_cache_service: RecipeCacheService,
        sample_recipe: UserRecipe,
        user_id: str,
        ...
        with pytest.raises(ValueError):
            await recipe_cache_service.update_recipe_field(
                user_id,
                thread_id,
                UpdateRecipeFieldParams(
                    id=recipe_id,
...
class TestUpdateRecipe:
    ...
    async def test_update_recipe_with_invalid_field_value(
        ...
        with pytest.raises(ValueError):
            await recipe_cache_service.update_recipe(
                user_id,
                thread_id,
                UpdateRecipeParams(
                    id=recipe_id,
    ...
    @pytest.mark.asyncio
    async def test_update_recipe_with_ingredients(
        self,
        recipe_cache_service: RecipeCacheService,
        sample_recipe: UserRecipe,
        user_id: str,
        ...
        new_ingredient = RecipeIngredient(name="new_ingredient_name", quantity="1", unit="unit")
    ...
    @pytest.mark.asyncio
    async def test_update_recipe_with_instructions(
        self,
        recipe_cache_service: RecipeCacheService,
        sample_recipe: UserRecipe,
        user_id: str,
        ...
        new_instruction = RecipeInstruction(
            title="new_instruction_title", description="new_instruction_description"
        )
    ...
    @pytest.mark.asyncio
    async def test_update_recipe_with_categories(
        self,
        recipe_cache_service: RecipeCacheService,
        sample_recipe: UserRecipe,
        user_id: str,
        ...
        new_category = RecipeCategory(name="new_category_name")
