...
class DummyModel:
    ...
    def __eq__(self, other: Any) -> bool:
        return isinstance(other, DummyModel) and self.__dict__ == other.__dict__
...
class TestRedisCache:
    ...
    async def test_get_ttl(self, redis_cache: RedisCache) -> None:
        key = "test:ttl"
    ...
    async def test_set_and_get_json(self, redis_cache: RedisCache) -> None:
        key = "test:json"
    ...
    async def test_set_json_with_ttl(self, redis_cache: RedisCache) -> None:
        key = "test:ttl"
    ...
    async def test_set_and_get_json_with_model(self, redis_cache: RedisCache) -> None:
        key = "test:json:model"
        data = {"id": "1", "value": "test"}
        await redis_cache.set_json(key, data)
    ...
    async def test_set_json_keep_ttl(self, redis_cache: RedisCache) -> None:
        key = "keep:ttl"
        data = {"id": "1", "value": "test"}
    ...
    async def test_fails_if_set_json_keep_ttl_with_ttl(self, redis_cache: RedisCache) -> None:
        key = "keep:ttl"
        data = {"id": "1", "value": "test"}
        with pytest.raises(ValueError):
            await redis_cache.set_json(key, data, ttl=5, keep_ttl=True)

    async def test_get_non_existent_key(self, redis_cache: RedisCache) -> None:
        key = "non:existent:key"
        result = await redis_cache.get_json(key, dict)
        assert result is None

    async def test_get_all_json_by_keys(self, redis_cache: RedisCache) -> None:
        keys = ["k1", "k2", "k3"]
    ...
    async def test_get_all_json_by_pattern(self, redis_cache: RedisCache) -> None:
    ...
    async def test_exists(self, redis_cache: RedisCache) -> None:
        key = "exists:key"
    ...
    async def test_incr_and_decr(self, redis_cache: RedisCache) -> None:
        key = "counter"
    ...
    async def test_delete_by_pattern(self, redis_cache: RedisCache) -> None:
        keys = ["1", "2", "3"]
        for key in keys:
    ...
    async def test_expire(self, redis_cache: RedisCache) -> None:
        key = "expire:key"
        await redis_cache.set_json(key, {"id": "1", "value": "test"})
        assert await redis_cache.exists(key) is True
