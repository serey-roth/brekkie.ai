...
class TestGetAndSetThread:
    @pytest.mark.asyncio
    async def test_get_thread_key(self, thread_cache_service: ThreadCacheService) -> None:
    ...
    @pytest.mark.asyncio
    async def test_get_all_threads_key(self, thread_cache_service: ThreadCacheService) -> None:
        key = thread_cache_service._get_all_threads_key("user_id")
        assert key == "brekkie:chat_session:user_id:threads:*:metadata"

    @pytest.mark.asyncio
    async def test_get_and_set_thread(self, thread_cache_service: ThreadCacheService) -> None:
        thread = Thread(
            id="thread_id",
            user_id="user_id",
    ...
    @pytest.mark.asyncio
    async def test_get_and_set_thread_with_ttl(
        self, thread_cache_service: ThreadCacheService
    ) -> None:
        thread = Thread(
            id="thread_id",
            user_id="user_id",
    ...
    @pytest.mark.asyncio
    async def test_get_threads(self, thread_cache_service: ThreadCacheService) -> None:
        threads = [
            Thread(
                id="thread_id_1",
                user_id="user_id",
...
class TestIsThreadEmpty:
    @pytest.mark.asyncio
    async def test_is_thread_empty(self, thread_cache_service: ThreadCacheService) -> None:
        await thread_cache_service.create_thread(
            CreateThreadParams(
                id="thread_id",
                user_id="user_id",
        ...
        await thread_cache_service.update_thread(
            "user_id",
            UpdateThreadParams(
                id="thread_id",
                updated_at=datetime.now(timezone.utc),
...
class TestCountUserThreads:
    @pytest.mark.asyncio
    async def test_count_user_threads(self, thread_cache_service: ThreadCacheService) -> None:
        await thread_cache_service.create_thread(
            CreateThreadParams(
                id="thread_id_1",
                user_id="user_id",
...
class TestUpdateThread:
    @pytest.mark.asyncio
    async def test_update_thread(self, thread_cache_service: ThreadCacheService) -> None:
        await thread_cache_service.create_thread(
            CreateThreadParams(
                id="thread_id",
                user_id="user_id",
        ...
        updated_thread = await thread_cache_service.update_thread(
            "user_id",
            UpdateThreadParams(
                id="thread_id",
    ...
    @pytest.mark.asyncio
    async def test_update_non_existent_thread(
        self, thread_cache_service: ThreadCacheService
    ) -> None:
...
class TestResumeThread:
    @pytest.mark.asyncio
    async def test_resume_thread(self, thread_cache_service: ThreadCacheService) -> None:
        thread = Thread(
            id="thread_id",
            user_id="user_id",
            created_at=to_utc_isostring(datetime.now(timezone.utc)),
        ...
        resumed_thread = await thread_cache_service.resume_thread(
            "user_id",
            ResumeThreadParams(
                id="thread_id",
                resumed_at=resumed_at,
...
class TestGetPaginatedThreads:
    ...
    async def create_threads(
        self, thread_cache_service: ThreadCacheService, sample_threads: list[Thread], user_id: str
    ) -> list[Thread]:
    ...
    @pytest.mark.asyncio
    async def test_no_threads(
        self, thread_cache_service: ThreadCacheService, create_threads: list[Thread]
    ) -> None:
        result = await thread_cache_service.get_paginated_threads(
            GetUserThreadsParams(
    ...
    async def test_exclude_empty_threads(
        self, thread_cache_service: ThreadCacheService, create_threads: list[Thread], user_id: str
    ) -> None:
        result = await thread_cache_service.get_paginated_threads(
            GetUserThreadsParams(
                user_id=user_id,
    ...
    @pytest.mark.asyncio
    async def test_no_more_threads(
        self,
        thread_cache_service: ThreadCacheService,
        create_threads: list[Thread],
        user_id: str,
