...
export function RecipeMessageCard({
    ...
    const [recipe, setRecipe] = useState<UserRecipe | null>(
        recipeManager.getRecipe(recipeId) ?? null,
    );
    ...
    if (isGenerating) {
        ...
        return (
            <div
                className={`bg-contrast-subtle border-border/50 hover:border-border w-full cursor-pointer rounded-lg border p-4 shadow-sm transition-all duration-200 ease-in-out hover:shadow-md ${selectedRecipeId === recipeId ? 'border-accent scale-[1.02] border-2 shadow-md' : ''} ${isGenerating && !recipe?.name ? 'pointer-events-none cursor-not-allowed opacity-60' : ''}`}
                onClick={handleSelectRecipe}
            >
                <div className="flex flex-col gap-3">
                    <div className="flex items-center gap-3">
                        <LoaderCircle className="text-background-light h-5 w-5 animate-spin" />
